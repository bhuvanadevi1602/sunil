...
<head>
    <meta charset="utf-8" />
    <title>Sales Invoice Report | Sunil Traders</title>
                ...
                <div class="row">
                  ...
                  <table class="table tablesss break" id="invoice-table" style="border: 1px solid;">
                            <thead>
                                <tr>
                                    <th class="text-center pt-1 pb-1" colspan="10">
                                        <h4 style="line-height: 10px;"><b><u>SALES INVOICE REPORT </u></b></h4>
                                        </td>
                                </tr>
                                <tr>
                                ...
                                <tr class="table-title">
                                    ...
                                    <th class="text-center party-name" style="width: 130px;">PARTY NAME</th>
                                    <th class="text-center" style="width: 90px;">AMOUNT</th>
                            ...
                            <tbody>
                                ...
                                $co = "select count(*) as n from comboset_invoice_final where created_at >= '$from_date' and created_at <= '$to_date';";
                            ...
                            //   print_r($m<$k);die();
                                 ...
                                 for ($i = 1; $i <= $k; $i++) {
                                    //   $d++;
                                        ...
                                        $s1 = (($m - 1) * 18)+1;
                                        $ms=$m-1;
                                            $cos = "select * from (select * from comboset_invoice_final where created_at >= '$from_date' and created_at <= '$to_date' order by id asc limit $ms,$s) as subt ;";
...
$iscts = floatval(str_replace(',', '.', str_replace('.', '', $igct)));
                                            ...
                                            $ms=$m-1;
                                            $cos = "select SUM(total) as v,FORMAT(SUM(replace(sgst_amount, ',', '')), 2) as sgst,FORMAT(SUM(replace(cgst_amount, ',', '')), 2) as cgst,FORMAT(SUM(replace(igst_amount, ',', '')), 2) as igst,sum(net_amount) as net_amount from (select total,cgst_amount,igst_amount,sgst_amount,net_amount from comboset_invoice_final where created_at >= '$from_date' and created_at <= '$to_date' order by id asc limit $ms,$s) as subt;";
                                        //   print_r($cos);die();
                                           $execos = mysqli_query($conn, $cos);
...
<?php                                                } ?>
                                    ...
                                    <td class="text-center"></td> -->
                                                <?php
                                                $cos1 = "select SUM(total) as tot,FORMAT(SUM(replace(cgst_amount, ',', '')), 2)  as cgst,FORMAT(SUM(replace(sgst_amount, ',', '')), 2)  as sgst,FORMAT(SUM(replace(igst_amount, ',', '')), 2)  as igst,sum(net_amount) as net_amount from comboset_invoice_final where created_at >= '$from_date' and created_at <= '$to_date'";
                                                // $cos="select * from  comboset_invoice_final where created_at >= '2023-04-01' and created_at <= '2023-04-30'";
...
$iscts = floatval(str_replace(',', '.', str_replace('.', '', $igct)));
                                            ...
                                            $ms=$m-1;
                                            $cos = "select SUM(total) as tot, FORMAT(SUM(replace(cgst_amount, ',', '')), 2) as cgst, FORMAT(SUM(replace(sgst_amount, ',', '')), 2) as sgst, FORMAT(SUM(replace(igst_amount, ',', '')), 2) as igst, SUM(net_amount) as net_amount  from (select total,cgst_amount,igst_amount,sgst_amount,net_amount from comboset_invoice_final where created_at >= '$from_date' and created_at <= '$to_date' order by id asc limit $ms,$s) as subt;";
                                        //   print_r($cos);die();
                                           $execos = mysqli_query($conn, $cos);
                                            $nos = mysqli_fetch_assoc($execos);
                                            ?>
                                    ...
                                    <td class="text-center"></td> -->
                                                <?php
                                                $cos1 = "select SUM(total) as tot, FORMAT(SUM(replace(cgst_amount, ',', '')), 2) as cgst, FORMAT(SUM(replace(sgst_amount, ',', '')), 2) as sgst, FORMAT(SUM(replace(igst_amount, ',', '')), 2) as igst, FORMAT(SUM(replace(net_amount, ',', '')), 2) as net_amount from comboset_invoice_final where created_at >= '$from_date' and created_at <= '$to_date'";
                                                // $cos="select * from  comboset_invoice_final where created_at >= '2023-04-01' and created_at <= '2023-04-30'";
                                                $execos1 = mysqli_query($conn, $cos1);
                                                $recs = mysqli_fetch_assoc($execos1);
                                                ?>
...
$iscts = floatval(str_replace(',', '.', str_replace('.', '', $igct)));
                                            ...
                                            <?php 
                                                ...
                                                $d1=($m-1)*18;
$d=$d1+1;
                                            $cos = "select * from (select * from comboset_invoice_final where created_at >= '$from_date' and created_at <= '$to_date' order by id asc limit $s1,$s) as subt;";
                                            // $cos="select * from comboset_invoice_final where created_at >= '2023-04-01' and created_at <= '2023-04-30'";
                                            $execos = mysqli_query($conn, $cos);
                                            while ($rec = mysqli_fetch_assoc($execos)) {
                                                $totalAmount = $totalAmount + $rec['total'];
...
$iscts = floatval(str_replace(',', '.', str_replace('.', '', $igct)));
                                            ...
                                            <?php
                                             ...
                                             $cos = "select sum(total) as v,FORMAT(SUM(replace(sgst_amount, ',', '')), 2) as sgst,FORMAT(SUM(replace(cgst_amount, ',', '')), 2) as cgst,sumFORMAT(SUM(replace(igst_amount, ',', '')), 2) as igst,sum(net_amount) as net_amount from (select total,cgst_amount,igst_amount,sgst_amount,net_amount from comboset_invoice_final where created_at >= '$from_date' and created_at <= '$to_date' order by id asc limit $s1,$s) as subt;";
                                            $execos = mysqli_query($conn, $cos);
                                            $nos = mysqli_fetch_assoc($execos);
                                            ?>
                                            <tr class="table-titless font-bold">
