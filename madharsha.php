                     ...
                     <div class="row">
                        <div class="col-lg-12">
                            <div class="page-title-box bg-light-alt text-center bs">
                                <h5 class="m-1 p-1 bgg"><b>MADHARSHA BILLING</b></h5>
                            </div>
                        </div> 
                        <div class="col-lg-3">
                            <div class="detail-design">
                              ...
                              <div class="input-group mt-2">
                                <div class="input-group-text b">Address</div>
                        ...
                        <div class="col-lg-9">
                            ...
                            <div class="row mt-1">
                                ...
                                <div class="col-lg-2">
                                    <label class=""><b>HSN Code</b></label>
                                ...
                                <div class="col-lg-2">
                                    ...
                                    <input id="pieces" type="text" value="" name="pieces" class="cursor-center add_entry form-control cardss mt-2 " placeholder="" required>  
                ...
                success:function(result_job)
                {
                    if(result_job.status==1)
                    {
                            ...
                            for(var i=0;i<count;i++)
                            {
                                ...
                                $('#particulars_rows').append('<tr id="row_'+result_job.data[i].madharsha_id+'"><td id="td_particular">'+result_job.data[i].particular+'</td><td id="td_hsn">'+result_job.data[i].hsn+'</td><td id="td_meters">'+result_job.data[i].meters+'</td><td id="td_pieces">'+result_job.data[i].pieces+'</td><td id="td_total_meters">'+result_job.data[i].total_meters+'</td><td id="td_rate">'+result_job.data[i].rate+'</td><td id="td_amount_'+i+'">'+rest_amount.toFixed(2)+'</td><td><button class="btn btn-outline-primary btn-sm mb-1 mb-xl-0 remove_particular1" id="'+result_job.data[i].madharsha_id+','+result_job.data[i].bill_id+'">Remove</button></td></tr>')
                            ...
                            $('#meters_count').val(meters_count.toFixed(2));
                            ...
                            $('.remove_particular1').on('click',function(){
                                ...
                                $.ajax({
                                    ...
                                    type:'POST',
                                    dataType:'json',
                                    data:{'action':'delete_madharsha_bill_entry','id':id,'bill_id':bill_id,'yearpicker':yearpicker},
                                    success:function(result_job){
...
$('.print_bill').click(function(){
    ...
    // link.substring(link.lastIndexOf('=') + 1).replace(/[^a-z0-9,. ]/gi, '');
   
    // link.attr('href', link.attr('href') + '?id='+cus_id+'');
    var customer_name=encodeURIComponent($('#customer_name').val());
    ...
    var net_amount=$('#net_amount').val();
     ...
     window.open('madharsha_bill.php?action=bill_printed&bill_id='+bill_id+'&customer_name='+customer_name+'&customer_contact='+customer_contact+'&gst_num='+gst_num+'&address_1='+address_1+'&address_2='+address_2+'&city='+city+'&sub_total='+sub_total+'&discount_amount='+discount_amount+'&total_amount='+total_amount+'&cgst='+cgst+'&sgst='+sgst+'&igst='+igst+'&net_amount='+net_amount+'&appr_no='+appr_no+'&appr_date='+appr_date+'&lr_no='+lr_no+'&lr_date='+lr_date+'&terms='+terms+'&through='+through+'&pieces_count='+pieces_count+'&finacial_year='+yearpicker);
...
$('#save_refresh').on('click',function(){
    ...
    $.ajax({
        ...
        type:'POST',
        dataType:'json',
        data:{'action':'madharsha_bill_printed','bill_id':bill_id,'customer_name':customer_name,'customer_contact':customer_contact,'gst_num':gst_num,'address_1':address_1,'address_2':address_1,'city':city,'appr_no':appr_no,'appr_date':appr_date,'lr_no':lr_no,'lr_date':lr_date,'terms':terms,'through':through,'sub_total':sub_total,'discount_amount':discount_amount,'total_amount':total_amount,'cgst':cgst,'sgst':sgst,'igst':igst,'net_amount':net_amount,'yearpicker':yearpicker,'gst_type':gst_type},
        success:function(result_job)
        {
            if(result_job.status==1)
            {
...
//     {
    $('#bill_id,#yearpicker').on('change',function(){
        ...
        $.ajax({
            url:'ajax_request.php',
            type:'POST',
            data:{'action':'fetch_madharsha_bills','bill_id':bill_id,'yearpicker':yearpicker},
            dataType:'json',
            success:function(result_job){
                // console.log(result_job.status);
                $('#particular').focus();
                $('#customer_name').val('');
                $('#gst_num').val('');
                $('#contact').val('');
               ...
               if(result_job.data[0].discount!='')
                    ...
                    for(var i=0;i<count;i++)
                    {
                       ...
                       $('#particulars_rows').append('<tr id="row_'+result_job.data[i].madharsha_id+'"><td id="td_particular">'+result_job.data[i].particular+'</td><td id="td_hsn">'+result_job.data[i].hsn+'</td><td id="td_meters">'+meter.toFixed(2)+'</td><td id="td_pieces">'+result_job.data[i].pieces+'</td><td id="td_total_meters">'+result_job.data[i].total_meters+'</td><td id="td_rate">'+result_job.data[i].rate+'</td><td id="td_amount_'+i+'">'+rest_amount.toFixed(2)+'</td><td><button class="btn btn-outline-primary btn-sm mb-1 mb-xl-0 remove_particular" id="'+result_job.data[i].madharsha_id+','+result_job.data[i].bill_id+'">Remove</button></td></tr>')    
                        }
                    }
                    ...
                    //       if(dis==0){
                    //      $('#total_amount').val(subtot.toFixed(2));
                    //  }
                    //   else{
                    //       var disv = parseFloat(dis)+ parseFloat(subtot);
                    //  $('#total_amount').val(disv.toFixed(2));
                    //   }
